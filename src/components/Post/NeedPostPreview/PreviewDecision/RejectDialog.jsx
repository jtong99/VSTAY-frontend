...
function RejectDialog({ id, status, open, onClose, decision, posterID }) {
  const [sendMessage] = useSendMessage({
    peerId: posterID,
    message: `ADMIN NOTIFICATION: Your needing post is ${decision}, please check it on your profile page. Feel free to send me message!`,
  });
  const router = useRouter();
  const { t } = useTranslation(['topnav']);
  const { getToken } = useContext(AuthContext);
  const [fire, { loading }] = useFetch(`${path}/${id}`, {
    token: getToken(),
    method: 'PATCH',
  });
